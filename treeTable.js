...
function depthFirst(tree, kids, func) {
	function i_depthFirst(node) {
		if (node[kids]) {
			$.each(node[kids], function(i, child) {
				i_depthFirst(child)
			})
		}
		func(node)
...
function renderTree(tree, kids, id, attrs, renderer, tableAttributes) {
	...
	function render(node, parent) {
		...
		if (parent) {
			$(tr).attr('data-tt-parent-id', parent[id])
		}
		if (renderer) {
			renderer($(tr), node)
		}else if (attrs) {
			...
			$.each(node, function(key, value) {
				if (key != kids && key != id && key != 'data-tt-level')
	...
	function i_renderTree(subTree, kids, level, parent) {
		...
		$.each(subTree, function(i, node) {
			...
			if (node[kids]) {
				$.each(node[kids], function(i, child) {
					i_renderTree([ child ], kids, level + 1, node)
				})
			}
		})
	}
	i_renderTree(tree, kids, 1)
	if (tree[0])
		tree[0].maxLevel = maxLevel
	return table
}

function attr2field(nodes, attrs){
...
function treeTable(table){
	...
	dat  = attr2field(dat, ['data-tt-id', 'data-tt-parent-id'])
